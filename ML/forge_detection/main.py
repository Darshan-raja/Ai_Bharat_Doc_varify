...
GEMINI_API_KEY = ""
ACTIVE_GEMINI_KEY_ID = "unconfigured"
...
def get_active_gemini_key():
    # Reload .env each request so newly pasted key starts working immediately.
...
def ensure_gemini_configured():
    global GEMINI_API_KEY, ACTIVE_GEMINI_KEY_ID

    active_key = get_active_gemini_key()
    key_id = get_key_id(active_key)

    if active_key != GEMINI_API_KEY:
        genai.configure(api_key=active_key)
        GEMINI_API_KEY = active_key
        ACTIVE_GEMINI_KEY_ID = key_id

    if key_id not in api_usage_by_key:
        api_usage_by_key[key_id] = {
            "date": str(date.today()),
            "count": 0,
        }
...
def get_quota_status():
    ...
    key_id = ensure_gemini_configured()
    usage = api_usage_by_key[key_id]
